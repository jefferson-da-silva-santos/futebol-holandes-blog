    ...
    <div className="login-root">
      <div className="login-card">
        ...
        <p className="login-hint">
          <i className="bx bx-info-circle" /> Primeiro acesso? Use{" "}
          <code>POST /setup</code> para criar o admin.
        </p>
      </div>
    </div>
  );
              ...
              >
                {ICONS.map((ic) => (
                  <option key={ic} value={ic}>
                    {ic.replace("bx bx", "").replace(/-/g, " ").trim()}
                  </option>
                ))}
              </select>
            </div>
    ...
    >
      ...
      <div className="adm-row-info">
        <div className="adm-row-meta">
          <span className={`badge ${article.category.badgeClass}`}>
...
function AdminPanel({
  ...
  async function handleEdit(data: ArticleInput) {
    ...
    try {
      ...
      setArticles((p) =>
        p.map((a) => (a.id === editing.id ? normalizeArticle(updated) : a)),
      );
      setEditing(null);
      setView("list");
  ...
  async function handleToggle(article: Article) {
    try {
      const updated = await articlesApi.patch(article.id, {
        published: !article.published,
      });
      setArticles((p) =>
        p.map((a) => (a.id === article.id ? normalizeArticle(updated) : a)),
      );
      showToast(
          ...
          <div className="adm-header-right">
            ...
            <div className="adm-user-menu">
              ...
              >
                ...
                <i
                  className={`bx ${userMenuOpen ? "bx-chevron-up" : "bx-chevron-down"}`}
                />
              </button>
              {userMenuOpen && (
...
export default function Admin({ onExit }: { onExit: () => void }) {
  ...
  function handleLogin(u: AdminUser) {
    setUser(u);
  }
